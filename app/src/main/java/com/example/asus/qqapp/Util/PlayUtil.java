...
import com.example.asus.qqapp.playMusic.MusicService;
...
public class PlayUtil {
    ...
    public static void play(Context context, String musicPath) {
        ...
        player.reset();
        try {
            if (!SDCardUtil.isLocal(currentMusic.getSongname() + "")) {
                player.setDataSource(musicPath);
            } else {
                player.setDataSource(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "MyMusic" + File.separator + currentMusic.getSongname() + ".mp3");
            }
            player.prepareAsync();
        } catch (IOException e) {
            e.printStackTrace();
